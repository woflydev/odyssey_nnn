...
def crop_image(img):
		scaled_img = cv2.resize(
				img, (max(int(params.img_height * 4 / 3), params.img_width), params.img_height)
		)
		...
		return scaled_img[
				starty : starty + params.img_height, startx : startx + params.img_width, :
		]
...
def preprocess(img):
		...
		if params.img_channels == 1:
				...
				img = np.reshape(
						img, (params.img_height, params.img_width, params.img_channels)
				)
		img = img / 255.0
		return img
...
parser.add_argument(
		"-t", "--throttle", help="throttle percent. [0-100]%", type=int, default=50
)
parser.add_argument(
		"--turnthresh", help="throttle percent. [0-30]degree", type=int, default=10
)
...
parser.add_argument(
		"--fpvvideo", help="Take FPV video of DNN driving", action="store_true"
)
parser.add_argument(
		"--use_tensorflow",
		help="use the full tensorflow instead of tflite",
		action="store_true",
)
parser.add_argument(
		"--pre", help="preprocessing [resize|crop]", type=str, default="resize"
)
